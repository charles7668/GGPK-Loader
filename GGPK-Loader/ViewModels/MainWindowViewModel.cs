...
public partial class MainWindowViewModel(IFileService fileService, IMessageService messageService) : ViewModelBase
{
    ...
    private async Task OpenGgpkFile()
    {
        ...
        try
        {
            var rootNode = await BuildGgpkTreeAsync(filePath);

            GgpkNodes.Clear();
            GgpkNodes.Add(rootNode);

            var bundleTree = await BuildBundleTreeAsync(rootNode, filePath);
    ...
    private static async Task<GGPKTreeNode> BuildGgpkTreeAsync(string filePath)
    {
    ...
    private static async Task<GGPKTreeNode?> BuildBundleTreeAsync(GGPKTreeNode ggpkRootNode, string ggpkFilePath)
    {
    ...
    private static GGPKTreeNode? ProcessBundle(GGPKTreeNode bundleRootNode, string ggpkFilePath)
    {
        ...
        ParsePaths(bundleIndexInfo.PathReps, decompressedDirectory, newBundleRootNode);
    ...
    private static void ParsePaths(BundleIndexInfo.PathRepRecord[] pathReps, byte[] directory, GGPKTreeNode rootNode)
    {
        foreach (var rep in pathReps)
        {
            ProcessPathRep(rep, directory, rootNode);
        }
    }

    private static void ProcessPathRep(BundleIndexInfo.PathRepRecord rep, byte[] directory, GGPKTreeNode rootNode)
    {
    ...
    private static void AddPathToTree(GGPKTreeNode root, string path)
    {
